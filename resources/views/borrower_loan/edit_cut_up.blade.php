...
@endsection
@section('breadcrumbs')
{!! Breadcrumbs::render('cut_up_borrower_loan') !!}
@endsection
@section('page_header','តារាងកាត់ប្រាក់កម្ចីឡើង')
@section('page_description','កាត់ឡើងប្រាក់កម្ចី')
@section('content')
    ...
    <div class="portlet-body form">
        <!-- BEGIN FORM-->
        <form action="{{route('borrower_loan.cut_up',$borrower_loan[0]->id)}}" class="form-horizontal" method="POST" >
            {{ method_field('PATCH') }}
            {{csrf_field()}}
            <div class="form-body">
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-control"​ name="loan_id" id="loan_id" >
                            @foreach ($loans as $loan)
                                <option value="{{$loan->id}}" {{$loan->id==$borrower_loan[0]->loan_id?'selected readonly':''}}>{{$loan->loan_name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group last">
                    ...
                    <div class="col-md-4">
                        <div class="input-group">
                            ...
                            <input type="text" readonly class="form-control input-circle-left date-picker" value="{{date('d/m/Y',strtotime(date('m/d/Y'). ' +1 day'))}}" name="start_pay_date">
                            <span class="input-group-addon input-circle-right ">
...
<script>
    ...
    $('#borrower_id').prop('disabled', true);
